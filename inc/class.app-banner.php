...
class Smartymedia_AppBanner {
        ...
        if ( $ios_link) {
            ...
            if ($result) {
                $ios_data = array(
                    ...
                    'stars' => $result['averageUserRating']*20, //convert rating 0-5 to 0-100%
                );
    ...
    public static function getGooglePlayAppDetails($google_app_id) {
        ...
        $pattern = '<meta itemprop="(.*?)" content="(.*?)"/>';
        preg_match_all($pattern, $page, $info_matches, PREG_SET_ORDER);
        
        $info_array = array("price"=>"0");
        $info_keys = array(
                           "operatingSystem"=>"os",
                           "name"=>"name",
                           "url"=>"url",
        ...
        foreach($info_matches as $child) {
            $key = $child[1];
            $value = $child[2];
            $info_array[$info_keys[$key]] = $value;
        }
        // Change image size to 100px
        $info_array["image"] = $info_array["image"]."=s100";
        $info_array["stars"] = $info_array["stars"]*20;
